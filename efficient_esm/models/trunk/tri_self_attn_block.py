...
from efficient_esm.models.folding.misc import (
    Attention,
    Dropout,
    PairToSequence,
    ResidueMLP,
...
class TriangularSelfAttentionBlock(nn.Module):
    ...
    def forward(self, sequence_state, pairwise_state, mask=None, chunk_size=None, **__kwargs):
        ...
        pairwise_state = pairwise_state + self.row_drop(
            self.tri_att_start(pairwise_state, mask=tri_mask, chunk_size=None)
        )
        pairwise_state = pairwise_state + self.col_drop(
            self.tri_att_end(pairwise_state, mask=tri_mask, chunk_size=None)
        )
